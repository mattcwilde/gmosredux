...
int     firstsciext    {0,prompt="number of first science extension to fit"}
int     lastsciext    {0,prompt="number of first science extension to fit"}
#
...
begin
    ...
    string  l_fitcfunc
    int     l_fitcxord, l_fitcyord, nbad, n_i, n_start
    int     l_firstsciext, l_lastsciext
    ...
    l_gsigma=gsigma
    l_firstsciext=firstsciext; l_lastsciext=lastsciext
    ...
    while (fscan(scanfile, img) != EOF) {
        ...
        if (l_firstsciext !=0) {
             n_start=l_firstsciext
        } else {
             n_start=1 
        }
        for (n_i=n_start; n_i<=nexten; n_i+=1) {
